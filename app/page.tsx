      ...
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <div className="fade-in" style={{ textAlign: 'center', marginBottom: 48 }}>
          <h1 style={{
            ...
            Calculadora de<br/>
            <span className="highlight">Independência Financeira</span>
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 16 }}>
            Descubra o que você precisa faturar para viver do seu negócio
          </p>
        </div>

        {/* Input Principal */}
        <div className="card glow fade-in fade-in-delay-1" style={{ padding: 40, marginBottom: 24 }}>
          ...
          <input
            ...
            step="5000"
            value={ganhoDesejado}
            onChange={(e) => setGanhoDesejado(parseInt(e.target.value))}
          />
          <div style={{
            display: 'flex',
        ...
        <div style={{
          ...
          <div className={`card fade-in fade-in-delay-2 ${animate ? 'pulse' : ''}`} style={{ padding: 28 }}>
            <div className="label" style={{ marginBottom: 8 }}>Faturamento Necessário</div>
            <div className="number" style={{ fontSize: 28, marginBottom: 4 }}>
              {formatCurrency(faturamentoMensal)}
            </div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>
              {formatCurrency(faturamentoMensal * 12)}/ano
            </div>
          </div>
          ...
          <div className={`card fade-in fade-in-delay-2 ${animate ? 'pulse' : ''}`} style={{ padding: 28 }}>
            ...
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>
              {percLucro}% do faturamento
            </div>
          </div>

          <div className={`card fade-in fade-in-delay-3 ${animate ? 'pulse' : ''}`} style={{ padding: 28 }}>
            <div className="label" style={{ marginBottom: 8 }}>Você Retira</div>
            <div className="number highlight" style={{ fontSize: 28, marginBottom: 4 }}>
              {formatCurrency(ganhoDesejado)}
            </div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>
              {percGanhos}% do lucro
            </div>
          ...
          <div className={`card fade-in fade-in-delay-3 ${animate ? 'pulse' : ''}`} style={{ padding: 28 }}>
            ...
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>
              {100 - percGanhos}% do lucro
            </div>
          </div>
        </div>

        {/* Independência Financeira */}
        <div className={`card glow fade-in fade-in-delay-4 ${animate ? 'pulse' : ''}`} style={{
          padding: 40,
          textAlign: 'center',
          marginBottom: 24,
          ...
          <div className="label" style={{ marginBottom: 12 }}>
            Para viver de renda com {formatCurrency(ganhoDesejado)}/mês
          </div>
          <div className="number" style={{
            fontSize: 'clamp(32px, 8vw, 56px)',
            marginBottom: 8,
          }}>
            {formatCurrency(investimentoNecessario)}
          </div>
          <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', marginBottom: 24 }}>
            investidos a {percRendimento}% a.a.
          </div>
          ...
          <div style={{
            ...
            <div className="number highlight" style={{ fontSize: 32 }}>
              {formatYears(anosIndependencia)}
            </div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginTop: 4 }}>
              economizando {formatCurrency(economiaAnual)}/ano
            </div>
          </div>
        </div>
            ...
            <div style={{ display: 'grid', gap: 24 }}>
              <div>
                ...
                <input
                  ...
                  max="80"
                  value={percLucro}
                  onChange={(e) => setPercLucro(parseInt(e.target.value))}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  max="90"
                  value={percGanhos}
                  onChange={(e) => setPercGanhos(parseInt(e.target.value))}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  min="4"
                  max="20"
                  value={percRendimento}
                  onChange={(e) => setPercRendimento(parseInt(e.target.value))}
                />
              </div>
            </div>
          </div>
        ...
        <div style={{
          ...
          RECONECTA · Calculadora de Meta Financeira
        </div>
      </div>
    </div>
  );
